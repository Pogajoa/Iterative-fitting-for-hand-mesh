...
img_list2 = os.listdir('./concatenate/images/iterated')
img_list3 = os.listdir('./concatenate/images/provided')

img_list1.sort(key = lambda x:x[0])
img_list2.sort(key = lambda x:x[0])
img_list3.sort(key = lambda x:x[0])

for i in range(len(img_list1)):
    img1 = cv2.imread('./concatenate/images/original/' + img_list1[i])
    img2 = cv2.imread('./concatenate/images/original/' + img_list2[i])
    img3 = cv2.imread('./concatenate/images/original/' + img_list3[i])
